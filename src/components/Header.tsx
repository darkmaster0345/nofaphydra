    ...
    <header className="flex flex-col items-center justify-center gap-6 py-12 page-transition">
      <div className="relative group p-4">
        ...
        <div className="relative z-10 p-3 bg-card rounded-full border-2 border-primary shadow-[0_0_40px_hsl(var(--primary)/0.1)] transition-all duration-700 group-hover:shadow-[0_0_60px_hsl(var(--primary)/0.2)]">
          <FursanLogo className="w-24 h-24 fursan-logo-glow text-primary" />
        </div>

        {level && (
          <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-card text-primary text-[10px] font-black px-6 py-2 rounded-xl shadow-2xl border-2 border-primary animate-in fade-in zoom-in duration-700 whitespace-nowrap tracking-widest uppercase">
            {level.name}
          </div>
        )}
      </div>
